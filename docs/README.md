# Documentation ShareAzure

Index de toute la documentation du projet.

## Installation

| Document | Description |
|----------|-------------|
| [installation/INSTALL.md](installation/INSTALL.md) | **Guide d'installation complet** (Terraform + .env + IA) |
| [installation/setup.sh](installation/setup.sh) | Script d'installation automatique |

## Guides

| Document | Description |
|----------|-------------|
| [GETTING_STARTED.md](GETTING_STARTED.md) | Guide de demarrage complet |
| [PROJECT_SUMMARY.md](PROJECT_SUMMARY.md) | Resume du projet et metriques |

## Architecture et configuration

| Document | Description |
|----------|-------------|
| [ARCHITECTURE.md](ARCHITECTURE.md) | Diagrammes d'architecture |
| [AZURE_SETUP.md](AZURE_SETUP.md) | Configuration Azure detaillee |
| [CUSTOMIZATION.md](CUSTOMIZATION.md) | Guide de personnalisation |

## Fonctionnalites

| Document | Description |
|----------|-------------|
| [SHARE_FEATURE.md](SHARE_FEATURE.md) | Partage de fichiers avec liens temporaires |
| [PREVIEW_FEATURE.md](PREVIEW_FEATURE.md) | Preview de fichiers (images, PDF, video, audio, texte) |
| [ADVANCED_FEATURES.md](ADVANCED_FEATURES.md) | Fonctionnalites avancees v2.0 |
| [GUEST_ACCOUNTS.md](GUEST_ACCOUNTS.md) | Systeme de comptes invites |
| [AI_FEATURES.md](AI_FEATURES.md) | Fonctionnalites IA/Multimedia et section Decouvrir |

## Administration

| Document | Description |
|----------|-------------|
| [ADMIN_INTERFACE.md](ADMIN_INTERFACE.md) | Interface d'administration |

## Reference API

| Document | Description |
|----------|-------------|
| [API_EXAMPLES.md](API_EXAMPLES.md) | Exemples curl pour tester l'API |

## Voir aussi

- [README.md](../README.md) - Presentation generale du projet
- [CHANGELOG.md](../CHANGELOG.md) - Historique des versions
- [CLAUDE.md](../CLAUDE.md) - Instructions pour Claude Code
